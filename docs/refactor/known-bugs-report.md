...
- **Status:** FIXED (was: OPEN)
- **Fixed:** 27-09-2026 00:10 — Import page rebuilt on the fixed importer (`admin/org/user/import`: form, template, result summary; Bulk import button on Users list). Export and history routes removed (not needed for UAT; exporter broken — BUG-158). Feature tests: `tests/Feature/Org/UserBulkImportPageTest.php`. DEC-036.
...
- **Status:** FIXED (was: OPEN (documented only — needs a schema/product decision before any fix))
- **Fixed:** 26-09-2026 — migration `2026_09_26_120000_add_missing_columns_to_xlr8_booking_master` adds sale_type VARCHAR(15), final_data JSON, consultant VARCHAR(50) (indexed), buyer_type VARCHAR(50), accessories TEXT, segment_code VARCHAR(50) (indexed). Types were approved by the user; production is confirmed to lack them too. `status_kw` is not added (only the unused BookingStateService uses it). Run on local xlrm + xlrm_testing; other environments get it through the normal deploy after UAT. The crash-documenting tests were replaced by regression tests. Follow-ups: `update()` keeps the stored col_type if omitted; `store()` dd() replaced by log + rethrow; VOTF branch falls back to the linked enquiry (see BUG-161). DEC-025, DEC-027.
...
- **Modified:** 27-09-2026 00:10 — the only live caller (user export) was removed with its routes (DEC-036); the dead pivot relations remain for Track B clean-up.
