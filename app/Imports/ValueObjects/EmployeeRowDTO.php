...
final class EmployeeRowDTO
{
    ...
    {
        $person = new Person([
            'aadhaar_no' => $this->aadhaarNo ?: null,
    ...
    /**
     * Format check delegated to App\Rules\PanNumber's pattern; normalization
     * (trim/uppercase) delegated to IdentifierService, matching every other
     * PAN read path in the app.
     */
    private static function cleanPan(mixed $v): ?string
    {
        $s = app(IdentifierService::class)->normalizePan(self::cleanStr($v));
        if (! $s) {
            return null;
        }
     ...
     * Cast float → int → zero-padded 12-char string, then delegate to
     * IdentifierService for the shared digit-normalization logic.
     */
    public static function cleanAadhaar(mixed $v): ?string
    {
        ...
        $digits = app(IdentifierService::class)->normalizeAadhaar($str);

        return $digits !== null && strlen($digits) === 12 ? $digits : null;
    }

    /**
     * Excel stores mobile as float: 9XXXXXXXXX.0. Strip the Excel-specific
     * float/scientific-notation artifact first, then delegate to
     * IdentifierService::cleanMobile() for the shared country-code/leading-
    ...
    private static function cleanMobile(mixed $v): ?string
    {
        ...
        $str = (string) $v;
        if (str_contains(strtolower($str), 'e') || is_float($v + 0)) {
            $str = (string) (int) (float) $str;
        }

        return app(IdentifierService::class)->cleanMobile($str);
    }
