...
class EmployeeSheetImport implements SkipsEmptyRows, ToCollection, WithHeadingRow
{
    use CodeGenerator, MasterDataSeeder;
     ...
     * Previously used its own PAN-first order, its own inline PAN/Aadhaar
     * regex, and an 'EMP-<empcode>' fallback built via ltrim($empCode, 'BMPL-')
     * - ltrim() treats its second argument as a character mask, not a prefix,
     * so it could eat real leading characters of a differently-shaped code.
     */
    ...
    private function parseDate($v): ?string
    {
        ...
        if (is_numeric($v)) {
            try {
                return Date::excelToDateTimeObject((float) $v)
                    ->format('Y-m-d');
            } catch (\Throwable) {
                return null;
            }
