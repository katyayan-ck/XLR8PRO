   ...
   public static function colors(?string $variantCode = null): array
{
    ...
    public static function getPermitMap(): array
    {
        return Cache::remember('rto_permit_map', self::CACHE_TTL, function () {

            // Numeric key => DB code. Order defines the numeric key.
            // MUST stay stable forever so old quotations don't change meaning.
            $legacyOrder = [
                '1'  => ['code' => 'PRIVATEUC4WL',      'cat' => 'Private'],
                '2'  => ['code' => 'PRIVATEBH4WL',      'cat' => 'Private'],
                '3'  => ['code' => 'PRIVATEEV4WL',      'cat' => 'Private'],
                '4'  => ['code' => 'GOODSG4WL',         'cat' => 'Goods'],
                '5'  => ['code' => 'GOODSGEV4WL',       'cat' => 'Goods'],
                '6'  => ['code' => 'GOODSG3TON+4WL',    'cat' => 'Goods'],
                '7'  => ['code' => 'GOODSGEV3TON+4WL',  'cat' => 'Goods'],
                '8'  => ['code' => 'GOODSG3WL',         'cat' => 'Goods'],
                '9'  => ['code' => 'GOODSGEV3WL',       'cat' => 'Goods'],
                '10' => ['code' => 'TAXIT4WL',          'cat' => 'Passenger'],
                '11' => ['code' => 'TAXITEV4WL',        'cat' => 'Passenger'],
                '12' => ['code' => 'PASSENGERP3WL',     'cat' => 'Passenger'],
                '13' => ['code' => 'PASSENGERPEV3WL',   'cat' => 'Passenger'],
                '14' => ['code' => 'AMBULANCEMISC',     'cat' => 'Passenger'],
            ];

            $codes = array_column($legacyOrder, 'code');
            $rows = Keyvalue::where('keyword_code', 'RTO_PERMIT')
                ->where('status', 1)
                ->where('is_active', 1)
                ->whereIn('code', $codes)
                ->get(['code', 'value'])
            ...
            $map = [];
            foreach ($legacyOrder as $numericKey => $def) {
                if (isset($rows[$def['code']])) {
                    $map[(string) $numericKey] = $rows[$def['code']]->value;
                }
            }
            return $map;
        });
    ...
    public static function getPermitPricingMap(): array
    {
        return Cache::remember('rto_permit_pricing_map', self::CACHE_TTL, function () {

            // Same array — duplicated here on purpose so getPermitMap() stays untouched in its public shape.
            $legacyOrder = [
                '1'  => ['code' => 'PRIVATEUC4WL',      'cat' => 'Private'],
                '2'  => ['code' => 'PRIVATEBH4WL',      'cat' => 'Private'],
                '3'  => ['code' => 'PRIVATEEV4WL',      'cat' => 'Private'],
