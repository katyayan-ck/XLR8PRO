...
class BookingCrudController extends CrudController
{
    ...
    */
    ...
    /**
     * Display OTF Bookings with custom AG Grid listing
     * 
    ...
    public function otfBookings(Request $request)
    {
        ...
        // Get base query with all joins
        $query = $this->getBaseQuery();

        // Filter bookings that have OTF data
        $query->whereNotNull('bookings.final_data')
            ->where('bookings.final_data', '!=', '')
            ->where('bookings.final_data', '!=', 'null')
            ->whereIn('bookings.status', [1, 2, 6, 8]);
        ...
        $gridData = $paginatedBookings->map(function ($booking, $index) use ($paginatedBookings) {
            ...
            $mapped->action = '
            <div class="d-flex justify-content-center gap-2">
                <a href="' . $showUrl . '" class="btn btn-sm btn-primary" title="View">View</a>
                <a href="' . $otfUrl . '" class="btn btn-sm btn-success" title="OTF Form">OTF</a>
                <a href="' . $editUrl . '" class="btn btn-sm btn-info" title="Edit">Edit</a>
            </div>
        ';
        ...
        return view('admin.booking.otf-bookings', $this->data);
    }
}
