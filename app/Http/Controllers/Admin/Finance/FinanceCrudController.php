...
/**
 * NOTE (see known-bugs-report.md BUG-028): the List/Create/Update/Delete
 * Operation traits and ScopedCrud below are dead scaffold — setup() is never
 * overridden, so CRUD::setModel() is never called, and no route (Route::crud()
 * or manual) registers any of those operations. The only real, reachable
 * action on this controller is import() below, which this rollout gates with
 * a single new "finance.import" permission (not the usual view/create/edit/
 * delete set, since there is no CRUD screen to gate). Left the dead scaffold
 * untouched — deciding its fate is a separate, undecided question (BUG-028).
 */
class FinanceCrudController extends CrudController
{
    ...
    private function parseDate($raw, $gid = null, $row = null): ?string
    {
        ...
        if (is_numeric($raw)) {
            try {
                return Date::excelToDateTimeObject((float) $raw)
                    ->format('Y-m-d');
            } catch (\Exception $e) {
            }
        }
