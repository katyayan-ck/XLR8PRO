...
/**
 * NOTE (see known-bugs-report.md BUG-028, same shape as Finance/Insurance):
 * the List/Create/Update/Delete Operation traits and ScopedCrud below are
 * dead scaffold — setup() is never overridden, so CRUD::setModel() is never
 * called, and no route registers any of those operations. The only real,
 * reachable action is import() below, gated here with a single new
 * "rto.import" permission. Left the dead scaffold untouched.
 *
 * See also BUG-029: import() references an undefined $spreadsheetId (unlike
 * its Finance/Insurance siblings, which hardcode a real one) — this action is
 * reachable but functionally broken, independent of and in addition to the
 * permission gating added here. Not fixed, since the correct spreadsheet ID
 * is unknown and unsafe to guess.
 *
 * Distinct from App\Http\Controllers\Admin\Pricing\RtoRuleController, which
 * is an unrelated concern (RTO pricing/charge rules for the vehicle pricing
 * engine, not RTO status/registration record import).
 */
