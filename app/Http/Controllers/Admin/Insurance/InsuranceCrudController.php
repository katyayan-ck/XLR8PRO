...
/**
 * NOTE (see known-bugs-report.md BUG-028, same shape found in FinanceCrudController):
 * the List/Create/Update/Delete Operation traits and ScopedCrud below are dead
 * scaffold — setup() is never overridden, so CRUD::setModel() is never called,
 * and no route (Route::crud() or manual) registers any of those operations.
 * The only real, reachable action on this controller is import() below, which
 * this rollout gates with a single new "insurance.import" permission (not the
 * usual view/create/edit/delete set, since there is no CRUD screen to gate).
 * Left the dead scaffold untouched — deciding its fate is a separate,
 * undecided question, tracked once under BUG-028 for both controllers.
 *
 * Distinct from App\Http\Controllers\Admin\Pricing\InsuranceController, which
 * is an unrelated concern (insurance premium/rate pricing rules for the
 * vehicle pricing engine, not policy-record import).
 */
