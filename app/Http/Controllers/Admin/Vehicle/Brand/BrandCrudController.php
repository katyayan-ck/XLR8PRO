...
namespace App\Http\Controllers\Admin\Vehicle\Brand;
...
class BrandCrudController extends CrudController
{
     ...
     * override — POST /admin/brand falls through to CreateOperation's default,
    ...
    /**
     * NOTE: this bulk Excel-import method predates this session and is dead code —
     * no route anywhere points to it (confirmed via a repo-wide grep). Left untouched
     * (not gated with a permission check, since it is unreachable) rather than
     * silently wiring or removing something outside this batch's scope.
     */
