...
<script>
    ...
    // Define standard visible fields optimized for the Exchange view 
    const DEFAULT_VISIBLE_FIELDS = [
        'serial_no', 'x8_enquiry_no', 'x8_enquiry_date',
        'first_name', 'mobile',
        'segment_name', 'model_name', 'variant_name',
        'dms_enquiry_stage', 'followup_status', 'sc_code',
        'expected_price', 'offered_price', 'exchange_bonus', 'price_gap',
        'action'
    ];

    // Build Column Groups dynamically based on Enquiry specific fields
    const assignedFields = new Set([
        'serial_no', 'x8_enquiry_no', 'x8_enquiry_date', 'oem_enquiry_no', 'purchase_type', 'enquiry_type',
        'first_name', 'last_name', 'mobile', 'email', 'city', 'district', 'tehsil', 'territory',
        'segment_name', 'model_name', 'variant_name', 'color_name',
        'dms_enquiry_stage', 'cre_enquiry_stage', 'followup_status', 'sc_code', 'dealer_branch',
        'expected_price', 'offered_price', 'exchange_bonus', 'price_gap', 'action'
    ]);

    const otherFields = ALL_COLUMNS.filter(c => !assignedFields.has(c.field));

    const columnGroups = [
        {
            headerName: 'Enquiry Details',
            headerClass: 'ag-header-center',
            children: getCols(['serial_no', 'x8_enquiry_no', 'x8_enquiry_date', 'oem_enquiry_no', 'purchase_type', 'enquiry_type']).map(col => {
                if (col.field === 'serial_no' || col.field === 'x8_enquiry_no') col.pinned = 'left';
                return col;
            })
        },
        {
            ...
            headerClass: 'ag-header-center',
            children: getCols(['first_name', 'last_name', 'mobile', 'email', 'city', 'district', 'tehsil', 'territory'])
        },
        {
            headerName: 'Vehicle Info',
            headerClass: 'ag-header-center',
            children: getCols(['segment_name', 'model_name', 'variant_name', 'color_name'])
        },
        {
            headerName: 'Exchange Valuations',
            headerClass: 'ag-header-center',
            children: getCols(['expected_price', 'offered_price', 'exchange_bonus', 'price_gap']).map(col => {
        ...
        },
        {
            ...
            headerClass: 'ag-header-center',
            children: getCols(['dms_enquiry_stage', 'cre_enquiry_stage', 'followup_status', 'sc_code', 'dealer_branch'])
        },
        {
    ...
    const gridOptions = {
        ...
        onGridReady: params => {
            ...
            columnGroups.forEach(group => {
                if (group.children) {
                    group.children.forEach(child => { if (child.field) allFields.push(child.field); });
                }
            });
    ...
    document.getElementById('btnAllHeaders')?.addEventListener('click', () => {
        ...
        columnGroups.forEach(group => {
            if (group.children) {
                group.children.forEach(c => { if (c.field) allFields.push(c.field); });
            }
        });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const exportFileName = "{{ Str::slug($title) }}-" + new Date().toISOString().slice(0,10);
