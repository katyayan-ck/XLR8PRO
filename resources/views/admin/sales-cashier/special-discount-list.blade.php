@php
    $gridConfig = [
        'columns' => [
            ...
            ['field' => 'deposit_date', 'headerName' => 'Deposit Date'],
            ...
            ['field' => 'contact_no_2', 'headerName' => 'Contact No.'],
            ['field' => 'model', 'headerName' => 'Model'],
            ['field' => 'variant', 'headerName' => 'Variant'],
            ['field' => 'amount', 'headerName' => 'Amount'],
            ['field' => 'attached_voucher', 'headerName' => 'Attached Voucher'],
...
@section('title', 'Special Discount List')
            ...
            <div class="card">
                <div class="card-header bg-gradient-primary d-flex justify-content-between align-items-center flex-nowrap flex-md-nowrap flex-wrap gap-3">
                    <h2 class="card-title mb-0 fw-bold text-black text-nowrap">
                        {{ $title ?? 'Special Discount List' }}
                    </h2>
                    <div class="d-flex align-items-center gap-3 flex-nowrap">
                        <a href="{{ backpack_url('special-discount/create') }}" class="btn btn-blue btn-sm fw-bold shadow-sm">
                            <i class="la la-plus me-1"></i>Add Special Discount
                        </a>
                    </div>
                </div>
                            ...
                            <div class="position-relative d-inline-block">
                                ...
                                <div id="columnBubble"
                                    ...
                                    <div style="max-height:260px; overflow:auto;">
                                        <table class="table table-sm mb-0"><tbody id="columnBubbleBody"></tbody></table>
                                    </div>
                                </div>
                            </div>
...
const DEFAULT_FIELDS = ['serial_no',
            'deposit_date',
            'xceler8_booking_no',
            'votf_no',
            'invoice_date',
...
const columnDefs = [
    ...ALL_COLUMNS.filter(col => DEFAULT_FIELDS.includes(col.field)),
...
const gridOptions = {
    ...
    onGridReady: params => {
        ...
        gridApi.setColumnsVisible([...DEFAULT_FIELDS, 'action'], true);
...
function openColumnBubble() {
    ...
    ALL_COLUMNS.forEach(col => {
        ...
        if (['serial_no', 'action'].includes(col.field)) checkbox.disabled = true;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('exportCsv').addEventListener('click', () => {
        const visibleColumns = gridApi.getAllDisplayedColumns().map(c => c.getColDef()).filter(c => c.field && c.field !== 'action');
        const rows = [];
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Special Discount');
        XLSX.writeFile(wb, `special-discount-${new Date().toISOString().slice(0, 10)}.xlsx`);
    });
    ...
    document.getElementById('exportPdf').addEventListener('click', () => {
        ...
        const rows = [];
        gridApi.forEachNodeAfterFilterAndSort(node => rows.push(visibleColumns.map(c => node.data[c.field] ?? '')));
        doc.autoTable({ head:[headers], body:rows, styles:{fontSize:8}, headStyles:{fillColor:[41,128,185]} });
        doc.save(`special-discount-${new Date().toISOString().slice(0, 10)}.pdf`);
    });
});
</script>
@endpush
