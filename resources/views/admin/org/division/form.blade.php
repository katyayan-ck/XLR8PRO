...
@section('title', $title ?? 'Division Form')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header text-black">
                    <h2 class="mb-0">
                        {{ isset($division) ? 'Edit Division Information' : 'Add New Division' }}
                    </h2>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form method="POST" action="{{ isset($division) ? backpack_url('org/division/' . $division->id) : backpack_url('org/division') }}" enctype="multipart/form-data">
                        @csrf
                        @if(isset($division))
                            @method('PUT')
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="dept_code" class="form-control form-select" required>
                                    ...
                                    <?php foreach($departments as$department): ?>
                                        <option value="{{ $department->code }}" {{ old('dept_code', $division->dept_code ?? '') == $department->code ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4 mb-3">
                                <label>
                                    Division Code
                                    @if(!isset($division)) <span class="text-danger">*</span> @endif
                                </label>
                                ...
                                @if(isset($division))
                                    ...
                                    <input type="hidden" name="code" value="{{ $division->code }}">
                            ...
                            <div class="col-md-8 mb-3">
                                ...
                                <textarea name="description" class="form-control" rows="1">{{ old('description', $division->description ?? '') }}</textarea>
                            </div>

                            {{-- ACTIVE --}}
                            @php
                                $oldDeptCode = old('dept_code',$division->dept_code ?? '');
                                $selectedDepartment =$departments->firstWhere('code', $oldDeptCode);$isDepartmentInactive = $selectedDepartment && !$selectedDepartment->is_active;
                            @endphp
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <div class="form-check form-switch">
                                    ...
                                    <input type="checkbox" id="divisionStatus" name="is_active" value="1" class="form-check-input" 
                        ...
                        @include('admin.org.partials.media-fields', ['imageCollection' => 'division_image', 'model' => $division ?? null])

                        <div class="mt-4">
                            <button type="submit" class="btn btn-success btn-lg px-5">
                                <i class="la la-save"></i> 
                                {{ isset($division) ? 'Update Division' : 'Create Division' }}
                            </button>
