            ...
            <div class="card-body p-0" style="background:#f8fafc">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 p-3 border-bottom bg-white">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        <a href="{{ backpack_url('testdrive/create') }}" class="btn btn-sm btn-primary d-flex align-items-center gap-2">
                            <i class="la la-plus"></i> <span class="d-none d-md-inline">Schedule</span>
                        </a>
...
<script>
    ...
    // Set Default Visible Fields for Test Drive Dashboard
    const DEFAULT_VISIBLE_FIELDS = [
        'serial_no', 'test_drive_no', 'enquiry_no',
        'customer_name', 'customer_phone',
        'model', 'variant',
        'stage', 'sc_code', 'scheduled_td_start_time', 'actual_td_start_time',
        'action'
    ];

    // Build Column Groups dynamically based on Test Drive fields
    ...
    const columnGroups = [
        {
            headerName: 'Primary Details',
            headerClass: 'ag-header-center',
            children: getCols(['serial_no', 'test_drive_no', 'enquiry_no']).map(col => {
                if (col.field === 'serial_no' || col.field === 'test_drive_no') col.pinned = 'left';
                return col;
            })
        },
        {
            ...
            headerClass: 'ag-header-center',
            children: getCols(['customer_name', 'customer_phone'])
        },
        {
            headerName: 'Vehicle',
            headerClass: 'ag-header-center',
            children: getCols(['model', 'variant'])
        },
        {
            headerName: 'Status & Timings',
            headerClass: 'ag-header-center',
            children: getCols(['stage', 'sc_code', 'scheduled_td_start_time', 'actual_td_start_time'])
        },
        {
        ...
        },
        {
            ...
            children: getCols(['action']).map(col => {
                ...
                // Dynamic Width Logic: Remove fixed width and let ag-grid autoSize handle it
    ...
    const gridOptions = {
        ...
        onGridReady: params => {
            ...
            // Trigger dynamic auto-sizing for all columns including Action
            setTimeout(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const exportFileName = "{{ Str::slug($title ?? 'test-drive') }}-" + new Date().toISOString().slice(0,10);
        ...
        document.getElementById('exportPdf')?.addEventListener('click', () => {
            ...
            doc.text('{{ $title ?? "Test Drive Report" }}', 40, 30);
            doc.autoTable({
                columns: exportCols,
                body: rows,
                startY: 50,
